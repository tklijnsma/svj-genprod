...
class FullSimRunnerBase(object):
    ...
    def for_year(cls, config, *args, **kwargs):
        config = svj.genprod.Config.flexible_init(config)
    ...
    def __init__(self, config, in_file, n_events):
        ...
        self.seed = svj.genprod.SVJ_SEED if self.__class__.seed is None else self.__class__.seed
        ...
        self.fullsim_dir = svj.genprod.RUN_FULLSIM_DIR
    ...
    def edit_cmsdriver_rnd_service(self):
        """
        Opens the config file generated by cmsDriver, and adds a few lines to
        ensure the rng is reset properly
        """
        logger.warning('Trying to add rnd service lines to cfg - fragile code')
        ...
        if n_repl_made == 0:
            raise ValueError(
                'Cannot add the rnd service lines to the cfg: regex substitution failed'
                )
    ...
    def stageout(self, stageout_directory=None):
        ...
        if stageout_directory is None:
            ...
            if svj.genprod.BATCH_MODE:
                ...
                condor_process_id = ''
            datestr = os.environ.get('CLUSTER_SUBMISSION_TIMESTAMP_SHORT', strftime('%y-%m-%d'))
            stageout_directory = (
        ...
        semanager = svj.genprod.SEManager()
