...
class GenSimFragment(object):
    ...
    def from_yaml(cls, yaml_file):
        config = svj.genprod.Config.from_yaml(yaml_file)
        return cls(config)

    @classmethod
    def from_file(cls, file):
        config = svj.genprod.Config.from_file(file)
        return cls(config)
    ...
    def get_lambda_d(self):
        ...
        if isinstance(self.alpha_d, str):
            _Lambda_d = svj.genprod.cdp.calc_lambda_d_from_str(self.n_c, self.n_f, self.alpha_d, self.m_dark_meson)
        else:
            _Lambda_d = svj.genprod.cdp.calc_lambda_d(self.n_c, self.n_f, self.alpha_d)
        ...
        #if Lambda_d < m_d:
        #    Lambda_d = 1.1 * m_d
        #    alpha_d = svj.genprod.cdp.calc_alpha_d(n_c, n_f, Lambda_d)
